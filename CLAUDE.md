...
`web/cookie-banner.js` **ya esta enlazado y activo en `web/index.html`** (banner de cookies propio, conforme LSSI-CE/RGPD y guia AEPD). Categorias: Necesarias (siempre) / Analisis (Google Analytics) / Google Maps, alineadas con `pages/politica-cookies.html`. Puntos clave del diseno: (1) los iframes de Google Maps **no cargan hasta el consentimiento** — el `src` viaja en `data-src` + `data-cookieconsent="google-maps"` y el iframe arranca con `hidden`; mientras tanto se muestra un placeholder (`.clinic-map__placeholder`, estilos en `styles.css`) con boton "Cargar mapa" (`[data-accept-maps]` = consentir solo mapas); (2) botones **Aceptar y Rechazar con el mismo peso visual** (requisito AEPD de igualdad); (3) el consentimiento se guarda en `localStorage` (`hsdental_cookie_consent`) y **caduca a los 365 dias**; (4) se puede reabrir/retirar desde el enlace **"Configuracion de cookies"** del footer (`#open-cookie-settings` -> `window.HSDentalCookies.openSettings()`). El banner inyecta sus propios estilos (autonomo) y usa Montserrat para armonizar con el sitio. Solo esta en `index.html` (las paginas legales de `web/pages/` no instalan cookies no necesarias, asi que no lo necesitan). Paginas legales en `web/pages/` (politica-cookies, politica-privacidad, aviso-legal, terminos-uso).
